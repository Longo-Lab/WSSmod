...
insertion into the frozen, sorted `train_vals` reference: its rank is
the count of reference values at or below it (ties in `train_vals` share
the same rank), and that rank is converted to a normal quantile using
the same Blom-type constant (`k = 0.375`) as
[`RNOmni::RankNorm()`](https://rdrr.io/pkg/RNOmni/man/RankNorm.html).

## Usage

``` r
project_rank_norm(train_vals, new_vals, k = 0.375)
```
...
- train_vals:
  ...
  Numeric vector, the frozen reference distribution (e.g. a baseline
  cohort's biomarker values). Must not contain `NA`.

- new_vals:

  Numeric vector of new values to project onto `train_vals`. May be a
  single value.

- k:

  Blom-type constant, passed through to match
  [`RNOmni::RankNorm()`](https://rdrr.io/pkg/RNOmni/man/RankNorm.html)'s
  default.

## Value

A numeric vector the same length as `new_vals`, giving each value's
projected normal quantile.
...
#> [1] -1.0271543 -0.1248768  0.8527665
```
